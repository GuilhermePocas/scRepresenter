...
def test_recon(model,x, data, knn_edge_index):
    ...
    model.eval()
    with torch.no_grad():
        embbed_rows, _, _, _ = model(x, knn_edge_index, data.train_pos_edge_index)
...
def crate_knn_batch(knn,idxs,k=15):
  ...
  if isinstance(idxs, torch.Tensor):
      idxs = idxs.detach().cpu().numpy()  # Ensures it's not a tensor

  elif isinstance(idxs, list):
      idxs = np.array(idxs)  # Convert list to NumPy array

  assert isinstance(idxs, np.ndarray), f"Error: idxs is {type(idxs)}, expected numpy.ndarray"
...
def train(data, loader, highly_variable_index, save_dir ,number_of_batches=5 ,
    ...
    for epoch in tqdm(range(max_epoch), desc="Training", total=max_epoch):
        ...
        lambda_clf_dynamic = lambda_clf * (1 / (1 + np.exp(-10 * (epoch / max_epoch - 0.8))))
        lambda_rows_dynamic = 1 - lambda_clf_dynamic
        lambda_cols_dynamic = 1 - lambda_clf_dynamic
        ...
        for i,batch in enumerate(loader):
            ...
            if cell_flag or knn_edge_index.shape[1] == loader.dataset.edge_index.shape[0] // number_of_batches :
                ...
                loss, row_loss, col_loss = model.calculate_loss(x.clone().to(device), knn_edge_index.to(device),
                                                                ...
                                                                lambda_cols=lambda_cols_dynamic, lambda_rows=lambda_rows_dynamic)


                with torch.no_grad():
                  if cell_flag:
                    row_embed, col_embed, out_features, embbed = model(x.clone().to(device), knn_edge_index, data.train_pos_edge_index)
                ...
                if num_classes>0:
                  ...
                  final_loss = loss + lambda_clf_dynamic*clf_loss
                  final_loss.backward()
        ...
        if epoch % 100 == 0:
          ...
          print(f"  Classifier Loss: {epoch_clf_loss/number_of_batches:.4f}")
          print(f"  Final Loss: {epoch_final_loss/number_of_batches:.4f}")
          print(f"  AUC: {auc:.4f}")
        ...
        if epoch%10 == 0:
          ...
          if cell_flag:
            ...
            save_obj(torch.concat(col_emb_lst).cpu().detach().numpy(), os.path.join(save_dir, f"col_embedding_{model_name}"))
            save_obj(row_embed.cpu().detach().numpy(), os.path.join(save_dir, f"row_embedding_{model_name}"))
            save_obj(torch.concat(out_features_lst).cpu().detach().numpy(), os.path.join(save_dir, f"out_features_{model_name}"))
            
          else:
            save_obj(new_knn_edge_index.cpu(), os.path.join(save_dir, f"best_new_knn_graph_{model_name}"))
            save_obj(col_embed.cpu().detach().numpy(), os.path.join(save_dir, f"col_embedding_{model_name}"))
            save_obj(row_embed.cpu().detach().numpy(), os.path.join(save_dir, f"row_embedding_{model_name}"))
            save_obj(out_features.cpu().detach().numpy(), os.path.join(save_dir, f"out_features_{model_name}"))
...
def mini_batch_cells(x,edge_index, batch_size, labels):
    cell_dataset = CellDataset(x, edge_index, labels=labels)
...
def run_scNET(ann_file, train_obj, save_dir, pre_processing_flag = True ,biogrid_flag = False,
    ...
    if pre_processing_flag:
      ...
      if train_obj.raw is None:
        train_obj.raw = train_obj.copy()
      sc.pp.log1p(train_obj)
      train_obj.X = train_obj.raw.X
      sc.pp.neighbors(train_obj, n_neighbors=n_neighbors, n_pcs=15)

    if clf_loss:
      labels = pd.Categorical(train_obj.obs["celltype"]).codes
    ...
    if split_cells:
      ...
      if highly_variable_index.sum() < 1200 or highly_variable_index.sum() > 5000:
        ...
        highly_variable_index = train_obj.var["std"]  >= train_obj.var["std"].sort_values(ascending=False)[train_obj.shape[1]-1]
      
      print(f"Highly variable genes: {highly_variable_index.sum()}")
  
    else:
      train_obj = train_obj[:,node_feature.index]
      knn_edge_index, highly_variable_index = build_knn_graph(train_obj)
    ...
    save_obj(node_feature, os.path.join(save_dir, f"node_features_{model_name}"))

    ids = pd.Series(train_obj.obs_names)
    save_obj(ids.to_numpy(), os.path.join(save_dir, f"ids_{model_name}"))
