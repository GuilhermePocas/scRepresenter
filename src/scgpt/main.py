...
def run_scGPT(model_name, hyperparameter_defaults, adata, save_dir):
    ...
    def train(model: nn.Module, loader: DataLoader, global_step: int) -> Tuple[Dict, Dict, int]:
        ...
        for batch, batch_data in enumerate(loader):
            ...
            batch_cell_names = batch_data["cell_names"]
            ...
            with torch.cuda.amp.autocast(enabled=config["amp"]):
                ...
                output_embeddings = output_dict["cell_emb"] 
                for name, emb in zip(batch_cell_names, output_embeddings):
                    embeddings_dic[name] = emb.detach().cpu().numpy()  # numpy array for saving

                output_cls = output_dict["cls_output"]
                for name, cls in zip(batch_cell_names, output_cls):
                    cls_dic[name] = cls.detach().cpu().numpy()  # numpy array for saving
        ...
        return embeddings_dic, cls_dic, global_step
        ...
        with torch.no_grad():
            for batch_data in loader:
                ...
                with torch.cuda.amp.autocast(enabled=config["amp"]):
                    ...
                    output_embeddings = output_dict["cell_emb"] 
                    for name, emb in zip(batch_cell_names, output_embeddings):
                        embeddings_dic[name] = emb.cpu().numpy()  # numpy array for saving

                    output_cls = output_dict["cls_output"]
                    for name, cls in zip(batch_cell_names, output_cls):
                        cls_dic[name] = cls.cpu().numpy()  # numpy array for saving
    ...
    def test(model: nn.Module, adata: DataLoader) -> float:
        ...
        model.eval()
        predictions, embeddings, cls = evaluate(
            model,
            loader=test_loader,
            return_raw=True,
        )
    ...
    with open(save_dir / "test_cell_embeddings.pkl", "wb+") as f:
